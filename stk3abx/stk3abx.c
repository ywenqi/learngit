/*
 *  stk3abx.c - Linux kernel modules for sensortek stk301x, stk321x, stk331x 
...
#define STK_WAIT1_REG                    0x05
#define STK_WAIT2_REG                    0x0F
...
#define STK_DATA1_IR_REG                0x13
#define STK_DATA2_IR_REG                0x14
#define STK_PDT_ID_REG                  0x3E
#define STK_RSRVD_REG                   0x3F
#define STK_SW_RESET_REG                0x80

#define STK_WAIT_16                     0x0A
#define STK_WAIT_20                     0x0C
#define STK_WAIT_50                     0x20
#define STK_WAIT_100                    0x40
#define STK_WAIT2_MAKS                  0x0F
#define STK_WAIT_H(X)                   ((X>>8) & STK_WAIT2_MAKS)
#define STK_WAIT_L(X)                   (X & 0xFF)
...
#ifdef STK_TUNE0
        #define STK_MAX_MIN_DIFF        100//60	//100
        #define STK_LT_N_CT     80//120
        #define STK_HT_N_CT     170	//50
...
#define PS_NAME "proximity"
        ...
        #define STK3ABX_VDD_MIN_UV      2000000
        #define STK3ABX_VDD_MAX_UV      3300000
        #define STK3ABX_VIO_MIN_UV              1750000
        #define STK3ABX_VIO_MAX_UV      1950000
#endif
#ifdef SPREADTRUM_PLATFORM
extern int sprd_3rdparty_gpio_pls_irq;

static struct stk3abx_platform_data stk3abx_pfdata={ 
  .state_reg = 0x0,    /* disable all */ 
  .psctrl_reg = 0x52,    /* ps_persistance=1, ps_gain=64X, PS_IT=0.391ms */ 
  .alsctrl_reg = 0x31,  /* als_persistance=1, als_gain=64X, ALS_IT=50ms */
  .ledctrl_reg = 0x10,   /* 100mA IRDR, 64/64 LED duty */ 
  .wait1_reg = STK_WAIT_L(STK_WAIT_100),     
  .wait2_reg = STK_WAIT_H(STK_WAIT_100),     
  .ps_thd_h = 3000, 
  .ps_thd_l = 2800, 
...
struct stk3abx_data {
        ...
        struct stk3abx_platform_data *pdata;
    ...
    struct work_struct stk_ps_work;
        ...
        //ywqstruct wake_lock ps_nosuspend_wl;
#endif
...
static struct i2c_client *stk3abx_i2c_client = NULL;

static int32_t stk3abx_enable_ps(struct stk3abx_data *ps_data, uint8_t enable, uint8_t validate_reg);
static int32_t stk3abx_enable_als(struct stk3abx_data *ps_data, uint8_t enable);
static int32_t stk3abx_set_ps_thd_l(struct stk3abx_data *ps_data, uint16_t thd_l);
static int32_t stk3abx_set_ps_thd_h(struct stk3abx_data *ps_data, uint16_t thd_h);
static int32_t stk3abx_set_als_thd_l(struct stk3abx_data *ps_data, uint16_t thd_l);
static int32_t stk3abx_set_als_thd_h(struct stk3abx_data *ps_data, uint16_t thd_h);
static int32_t stk3abx_get_ir_reading(struct stk3abx_data *ps_data);
static uint8_t stk_esd_check(struct stk3abx_data *ps_data);
static int32_t stk_esd_recover(struct stk3abx_data *ps_data);

#ifdef STK_TUNE0
static int stk_ps_tune_zero_func_fae(struct stk3abx_data *ps_data);
#endif
#ifdef STK_CHK_REG
static int stk3abx_validate_n_handle(struct i2c_client *client);
#endif
static int stk_ps_val(struct stk3abx_data *ps_data);
#ifdef STK_QUALCOMM_POWER_CTRL
static int stk3abx_device_ctl(struct stk3abx_data *ps_data, bool enable);
#endif

static int stk3abx_i2c_read_data(struct i2c_client *client, unsigned char command, int length, unsigned char *values)
{
        uint8_t retry;
        int err;
        struct i2c_msg msgs[] = 
...
static int stk3abx_i2c_write_data(struct i2c_client *client, unsigned char command, int length, unsigned char *values)
{
        int retry;
        int err;
...
static int stk3abx_i2c_smbus_read_byte_data(struct i2c_client *client, unsigned char command)
{
        unsigned char value;
        int err;
        err = stk3abx_i2c_read_data(client, command, 1, &value);
        if(err < 0)
                return err;
        return value;
}

static int stk3abx_i2c_smbus_write_byte_data(struct i2c_client *client, unsigned char command, unsigned char value)
{
        int err;
        err = stk3abx_i2c_write_data(client, command, 1, &value);
        return err;
}



static int stk3abx_open(struct inode *inode, struct file *file)
{
	file->private_data = stk3abx_i2c_client;
...
static int stk3abx_release(struct inode *inode, struct file *file)
{
	file->private_data = NULL;
	return 0;
}
static int32_t stk3abx_check_pid(struct stk3abx_data *ps_data)
{
        unsigned char value[2], pid_msb;
        int err;

        err = stk3abx_i2c_read_data(ps_data->client, STK_PDT_ID_REG, 2, &value[0]);
        if(err < 0)
        {
        ...
        if(value[0] == 0)
        {
                printk(KERN_ERR "PID=0x0, please make sure the chip is stk3abx!\n");
                return -2;
        }
        ...
        case 0x30:
                case 0x40:
                case 0x50:
                                printk(KERN_ERR "%s: 1 OK PID(%#x)\n", __func__, value[0]);
                return 0;
        default:
                printk(KERN_ERR "%s: 2 invalid PID(%#x)\n", __func__, value[0]);
               // return -1;
        }
        return 0;
}

static long stk3abx_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
		//int value;
		int flag = 0;
		//unsigned char *buf;
		#define DEVICE_INFO_LEN         32
                                #define DEVICE_INFO             "STK3ABX"
                                u8 devid = 0;
                                u8 devinfo[DEVICE_INFO_LEN] = { 0 };
		void __user *argp = (void __user *)arg;
		struct i2c_client *client = file->private_data;
        	struct stk3abx_data *ps_data = i2c_get_clientdata(client);		
		...
		switch(cmd)
		{
			...
			case LTR_IOCTL_SET_PFLAG:
				...
				//ywqem_sensor_restart_work();
	
				if(flag==1){
					stk3abx_enable_ps(ps_data, 1, 0);	
				}
				else if(flag==0) {
					stk3abx_enable_ps(ps_data, 0, 0);	
				}
	
				//ywqelan_sensor_restart_work();
			...
			case LTR_IOCTL_SET_LFLAG:
				...
				if(flag==1){
					stk3abx_enable_als(ps_data, 1);
				}
				else if(flag==0) {
					stk3abx_enable_als(ps_data, 0);
				}
			...
			case LTR_IOCTL_GET_CHIPINFO:
                        	...
                        	if (copy_to_user(argp, "STK3ABX", sizeof("STK3ABX"))) {
...
static struct file_operations stk3abx_pls_fops = {
	.owner				= THIS_MODULE,
	.open				= stk3abx_open,
	.release			= stk3abx_release,
	.unlocked_ioctl				= stk3abx_unlocked_ioctl,
};
static struct miscdevice stk3abx_pls_device = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "stk3abx",
	.fops = &stk3abx_pls_fops,
};

uint32_t stk_alscode2lux(struct stk3abx_data *ps_data, uint32_t alscode)
{
        alscode += ((alscode<<7)+(alscode<<3)+(alscode>>1));   
    alscode<<=3; 

	printk(KERN_INFO "%s: alscode=%d\n",__func__,alscode);

    alscode/=ps_data->als_transmittance;
	alscode = alscode / 24 * 10;
        return alscode;
}

uint32_t stk_lux2alscode(struct stk3abx_data *ps_data, uint32_t lux)
{
...
static void stk_init_code_threshold_table(struct stk3abx_data *ps_data)
{
    uint32_t i,j;
    uint32_t alscode;
...
#else
void stk_als_set_new_thd(struct stk3abx_data *ps_data, uint16_t alscode)
{
    ...
    if (low_thd <0)
        low_thd = 0;
    stk3abx_set_als_thd_h(ps_data, (uint16_t)high_thd);
    stk3abx_set_als_thd_l(ps_data, (uint16_t)low_thd);
}
#endif // CONFIG_STK_PS_ALS_USE_CHANGE_THRESHOLD


static void stk3abx_proc_plat_data(struct stk3abx_data *ps_data, struct stk3abx_platform_data *plat_data)
{
        uint8_t w_reg;
...
#ifdef STK_POLL_PS
        ps_data->psctrl_reg &= 0x5F;
#endif
        ps_data->alsctrl_reg = plat_data->alsctrl_reg;
        ps_data->ledctrl_reg = plat_data->ledctrl_reg;

        ps_data->wait1_reg = plat_data->wait1_reg;
		ps_data->wait2_reg = plat_data->wait2_reg;
        if(ps_data->wait1_reg < 2)
        {
                printk(KERN_WARNING "%s: wait1_reg should be larger than 2, force to write 2\n", __func__);
                ps_data->wait1_reg = 2;
        }
        else if (ps_data->wait1_reg > 0xFF)
        {
                printk(KERN_WARNING "%s: wait1_reg should be less than 0xFF, force to write 0xFF\n", __func__);
                ps_data->wait1_reg = 0xFF;
        }
//#ifndef STK_TUNE0
...
static int32_t stk3abx_init_all_reg(struct stk3abx_data *ps_data)
{
        int32_t ret;

    ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, STK_STATE_REG, ps_data->state_reg);
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
    }
    ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, STK_PSCTRL_REG, ps_data->psctrl_reg);
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
    }
    ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, STK_ALSCTRL_REG, ps_data->alsctrl_reg);
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
    }
    ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, STK_LEDCTRL_REG, ps_data->ledctrl_reg);
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
    ...
    ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, 0xF4, 0x00);
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
    ...
    ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, STK_WAIT1_REG, ps_data->wait1_reg);
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
	...
	ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, STK_WAIT2_REG, ps_data->wait2_reg);
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
    ...
    ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, 0x4E, 0x50);
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
    ...
    ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, 0xAB, 0x64);
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
    ...
    ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, 0xA0, 0x10);
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
    ...
    ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, 0xA1, 0x03);
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
    ...
    ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, 0xDB, 0x15);
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
    ...
    ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, 0xE1, 0x10);
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
...
#endif
//#else
        stk3abx_set_ps_thd_h(ps_data, ps_data->ps_thd_h);
        stk3abx_set_ps_thd_l(ps_data, ps_data->ps_thd_l);
//#endif

    ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, STK_INT_REG, ps_data->int_reg);
    if (ret < 0)
        {
                printk(KERN_ERR "%s: write i2c error\n", __func__);
                return ret;
        ...
        /*
    ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, 0x87, 0x60);
    if (ret < 0)
        {
                printk(KERN_ERR "%s: write i2c error\n", __func__);
                return ret;
...
static int32_t stk3abx_software_reset(struct stk3abx_data *ps_data)
{
    int32_t r;
    uint8_t w_reg;

    w_reg = STK_WAIT_L(STK_WAIT_100);
    r = stk3abx_i2c_smbus_write_byte_data(ps_data->client,STK_WAIT1_REG,w_reg);
    if (r<0)
    {
        printk(KERN_ERR "%s: software reset: write i2c error, ret=%d\n", __func__, r);
        return r;
    }
    r = stk3abx_i2c_smbus_read_byte_data(ps_data->client,STK_WAIT1_REG);
    if (w_reg != r)
    {
    ...
    r = stk3abx_i2c_smbus_write_byte_data(ps_data->client,STK_SW_RESET_REG,0);
    if (r<0)
    {
...
static int32_t stk3abx_set_als_thd_l(struct stk3abx_data *ps_data, uint16_t thd_l)
{
        ...
        val[1] = thd_l & 0x00FF;
        ret = stk3abx_i2c_write_data(ps_data->client, STK_THDL1_ALS_REG, 2, val);
        if(ret < 0)
                printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
        return ret;
}
static int32_t stk3abx_set_als_thd_h(struct stk3abx_data *ps_data, uint16_t thd_h)
{
        ...
        val[1] = thd_h & 0x00FF;
        ret = stk3abx_i2c_write_data(ps_data->client, STK_THDH1_ALS_REG, 2, val);
        if(ret < 0)
                printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
        return ret;
}
...
static int32_t stk3abx_set_ps_thd_l(struct stk3abx_data *ps_data, uint16_t thd_l)
{
        ...
        val[0] = (thd_l & 0xFF00) >> 8;
        val[1] = thd_l & 0x00FF;
        ret = stk3abx_i2c_write_data(ps_data->client, STK_THDL1_PS_REG, 2, val);
        if(ret < 0)
                printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
        return ret;
}
static int32_t stk3abx_set_ps_thd_h(struct stk3abx_data *ps_data, uint16_t thd_h)
{
        ...
        val[0] = (thd_h & 0xFF00) >> 8;
        val[1] = thd_h & 0x00FF;
        ret = stk3abx_i2c_write_data(ps_data->client, STK_THDH1_PS_REG, 2, val);
        if(ret < 0)
                printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
        return ret;
}

static uint32_t stk3abx_get_ps_reading(struct stk3abx_data *ps_data)
{
        unsigned char value[2];
        int err;
		uint8_t esd_checked;
		uint16_t word_data;
		esd_checked = stk_esd_check(ps_data);
		
		if(esd_checked == 0xFF){
		
			printk(KERN_ERR "%s: ps esd checked\n", __func__);
			stk_esd_recover(ps_data);
			/*
			ret = stk3abx_i2c_read_data(ps_data->client, STK_DATA1_PS_REG, 2, &value[0]);
			if(ret < 0)
			{
			...
			printk(KERN_ERR "%s: ps_raw_data report after esd_recover=%d\n", __func__, word_data);
			return word_data;
			*/
		}
		
        err = stk3abx_i2c_read_data(ps_data->client, STK_DATA1_PS_REG, 2, &value[0]);
        if(err < 0)
        {
                printk(KERN_ERR "%s: fail, ret=%d\n", __func__, err);
                return err;
		...
		word_data = (value[0]<<8) | value[1];
		printk(KERN_ERR "%s: ps_raw_data =%d\n", __func__, word_data);
        return word_data;
}


static int32_t stk3abx_set_flag(struct stk3abx_data *ps_data, uint8_t org_flag_reg, uint8_t clr)
{
        uint8_t w_flag;
        int ret;
    ...
    ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client,STK_FLAG_REG, w_flag);
        if(ret < 0)
                printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
        return ret;
}

static int32_t stk3abx_get_flag(struct stk3abx_data *ps_data)
{
        int ret;
    ret = stk3abx_i2c_smbus_read_byte_data(ps_data->client,STK_FLAG_REG);
        if(ret < 0)
                printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
        return ret;
}
...
#ifdef STK_GES
static int32_t stk3abx_set_flag2(struct stk3abx_data *ps_data, uint8_t org_flag2_reg, uint8_t clr)
{
        uint8_t w_flag2;
        int ret;
    ...
    ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client,STK_FLAG2_REG, w_flag2);
        if(ret < 0)
                printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
        return ret;
}

static int32_t stk3abx_get_flag2(struct stk3abx_data *ps_data)
{
        int ret;
    ret = stk3abx_i2c_smbus_read_byte_data(ps_data->client,STK_FLAG2_REG);
        if(ret < 0)
                printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
        return ret;
}
...
static int32_t stk3abx_set_state(struct stk3abx_data *ps_data, uint8_t state)
{
        int ret;
    ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client,STK_STATE_REG, state);
        if(ret < 0)
                printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
        return ret;
}

static int32_t stk3abx_get_state(struct stk3abx_data *ps_data)
{
        int ret;
    ret = stk3abx_i2c_smbus_read_byte_data(ps_data->client,STK_STATE_REG);
        if(ret < 0)
                printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
        return ret;
}
...
#ifdef STK_GES
static int32_t stk3abx_set_gsctrl(struct stk3abx_data *ps_data, uint8_t state)
{
        int ret;
    ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client,STK_GSCTRL_REG, state);
        if(ret < 0)
                printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
        return ret;
}

static int32_t stk3abx_get_gsctrl(struct stk3abx_data *ps_data)
{
        int ret;
    ret = stk3abx_i2c_smbus_read_byte_data(ps_data->client,STK_GSCTRL_REG);
        if(ret < 0)
                printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
        return ret;
}
...
#if 0
static uint32_t stk3abx_get_ges_reading(struct stk3abx_data *ps_data, unsigned int *ges0,
                                unsigned int *ges1, unsigned int *ges2)
{
        ...
        while(stk_ges_op[no].ops[1] != 0)
        {
                ...
                if(stk_ges_op[no].action.rw_len_retry & 0x80)
                {
                        while(retry != 0)
                        {
                                err = stk3abx_i2c_read_data(ps_data->client, 
                ...
                else
                {
                        while(retry != 0)
                        {
                                err = stk3abx_i2c_write_data(ps_data->client, stk_ges_op[no].action.reg, 
                                                                len, &reg_val_crit);
                                if(err < 0)
                                {
...
#else
static uint32_t stk3abx_get_ges_reading(struct stk3abx_data *ps_data, unsigned int *ges0,unsigned int *ges1,unsigned int *ges2)
{
        unsigned char value[4];
        int err, retry = 10;

        do {
                err = stk3abx_get_flag(ps_data);
                if(err < 0)
                        return err;
        ...
        err = stk3abx_i2c_read_data(ps_data->client, 0x24, 4, &value[0]);
        if(err < 0)
        {
                printk(KERN_ERR "%s: fail, ret=%d\n", __func__, err);
                return err;
...
static int32_t stk3abx_enable_ges(struct stk3abx_data *ps_data, uint8_t enable, uint8_t mode)
{
    int32_t ret;
        ...
        {
#ifdef STK_QUALCOMM_POWER_CTRL
                ret = stk3abx_device_ctl(ps_data, enable);
                if (ret)
                        return ret;
#endif
                ...
                ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, STK_WAIT1_REG, 0);
                if (ret < 0)
                {
                        printk(KERN_ERR "%s: write i2c error\n", __func__);
                        return ret;
				...
				ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, STK_WAIT2_REG, 0);
                if (ret < 0)
                {
                        printk(KERN_ERR "%s: write i2c error\n", __func__);
                        return ret;
                ...
                ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, STK_INT_REG, 0);
                if (ret < 0)
                {
                        printk(KERN_ERR "%s: write i2c error\n", __func__);
                        return ret;
                ...
                ret = stk3abx_set_state(ps_data, w_state_reg);
                if(ret < 0)
                        return ret;

                if(mode == 2)
                {
                        ret = stk3abx_get_gsctrl(ps_data);
                        if(ret < 0)
                                return ret;
                        gsctrl_reg = ret & 0xF3;
#ifdef STK_POLL_PS
                        ...
                        gsctrl_reg |= 0x0C;
#endif
                        ret = stk3abx_set_gsctrl(ps_data, gsctrl_reg);
                        if(ret < 0)
                                return ret;
#ifdef STK_POLL_PS
        ...
        else
        {
                ...
                ret = stk3abx_set_state(ps_data, 0);
                if(ret < 0)
                        return ret;
...
static int32_t stk3abx_enable_ps(struct stk3abx_data *ps_data, uint8_t enable, uint8_t validate_reg)
{
    int32_t ret;
        ...
        if(validate_reg)
        {
                ret = stk3abx_validate_n_handle(ps_data->client);
                if(ret < 0)
                        printk(KERN_ERR "stk3abx_validate_n_handle fail: %d\n", ret); 
        }
        ...
        if(ps_data->ges_enabled && enable)
        {
                ...
                stk3abx_enable_ges(ps_data, 0, 1);
    ...
    if(enable)
        {
                ...
                if(!validate_reg)
                {
                        ...
                        reading = stk3abx_get_ps_reading(ps_data);
...
#endif /* #ifdef STK_CHK_REG */
                {
                        ...
                        input_sync(ps_data->ps_input_dev);
                        //ywqwake_lock_timeout(&ps_data->ps_wakelock, 3*HZ);
						__pm_wakeup_event(ps_data->ps_wakelock, 3*HZ);
                        reading = stk3abx_get_ps_reading(ps_data);
        ...
        else
        {
                ...
                if(ps_data->re_enable_ges)
                {
                        ...
                        stk3abx_enable_ges(ps_data, 1, 1);
...
static int32_t stk3abx_enable_als(struct stk3abx_data *ps_data, uint8_t enable)
{
    int32_t ret;
        ...
        {
                ret = stk3abx_get_ir_reading(ps_data);
                if(ret > 0)
                        ps_data->ir_code = ret;
        }
        #endif
    ...
    if (enable)
        {
        stk3abx_set_als_thd_h(ps_data, 0x0000);
        stk3abx_set_als_thd_l(ps_data, 0xFFFF);
        }
#endif
...
uint8_t stk_esd_check(struct stk3abx_data *ps_data)
{
	uint8_t reg[5] = {0x01, 0x02, 0x03, 0x05, 0x0F};
	uint8_t values[5] = {0x52, 0x31, 0x10, STK_WAIT_L(STK_WAIT_100), STK_WAIT_H(STK_WAIT_100)};
	uint8_t buf[2] = {0x00, 0x00};
	uint8_t state = 0x00;
	uint32_t ret = 0, i;
	
	if(ps_data->esd_checked != 0){
		printk(KERN_ERR "%s: esd =%d\n", __func__, ps_data->esd_checked);
		return 0xFF;
	}

	for(i = 0; i < 6; i++){
		if(i == 5){
			if(ps_data->als_enabled == 1){
				state = 0x02;
			}
			if(ps_data->ps_enabled == 1){
				if(ps_data->als_enabled == 1){
					state = 0x03;
				}else{
					state = 0x05;
			...
			buf[0] = 0x00;
			buf[1] = 0x00;
			ret = stk3abx_i2c_read_data(ps_data->client, 0x00, 1, &buf[1]);
			if(ret < 0)
			{
                printk(KERN_ERR "%s fail, ret=0x%x", __func__, ret);
                return ret;
			...
			if(buf[1] != state) {
				ps_data->esd_checked = 1;
				printk(KERN_ERR "%s esd checked reg[0x00]=0x%x\n",  __func__, buf[1]);
				break;
			}			
		}else{
			buf[0] = reg[i];
			buf[1] = 0x00;
			ret = stk3abx_i2c_read_data(ps_data->client, buf[0], 1, &buf[1]);
			if(ret < 0)
			{
                printk(KERN_ERR "%s fail, ret=0x%x", __func__, ret);
                return ret;
			...
			if(buf[1] != values[i]) {
				ps_data->esd_checked = 1;
				printk(KERN_ERR "%s esd checked reg[0x%x]=0x%x\n",  __func__, reg[i], buf[1]);
				break;
			}
		}
	}

	if(ps_data->esd_checked == 1){
		return 0xFF;
	}
...
int32_t stk_esd_recover(struct stk3abx_data *ps_data) 
{
	uint8_t buf[2] = {0x00, 0x00};
	int32_t ret = 0;
	...
	if(ps_data->esd_checked == 1){
			buf[0] = 0x80;
			buf[1] = 0x99;
			ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, 0x80, 0x99);
			if (ret < 0)
				{
					printk(KERN_ERR "%s: write i2c error\n", __func__);
					return ret;
				}
			printk(KERN_ERR "%s softreset\n", __func__);
			ps_data->esd_checked = 2;
	}else if(ps_data->esd_checked == 2){
				...
				printk(KERN_ERR "%s:stk esd init cfg\n", __func__);
				
				ps_data->esd_checked = 0;
	}else{
		ps_data->esd_checked = 0;
	}
	return ret;
...
static int32_t stk3abx_get_als_reading(struct stk3abx_data *ps_data)
{
    int32_t word_data;
#ifdef STK_ALS_FIR
        int index;   
		...
		if(esd_checked == 0xFF){
			...
			printk(KERN_ERR "%s: als esd checked\n", __func__);
			stk_esd_recover(ps_data);
			/*
			ret = stk3abx_i2c_read_data(ps_data->client, STK_DATA1_ALS_REG, 2, &value[0]);
			if(ret < 0)
			{
                printk(KERN_ERR "%s fail, ret=0x%x", __func__, ret);
                return ret;
			...
			word_data = (value[0]<<8) | value[1];
			printk(KERN_ERR "%s: alscode report after esd_recover=%d\n", __func__, word_data);
			return word_data;
			*/
		}
		...
		printk(KERN_ERR "%s: als_raw_data %d\n", __func__, word_data);
#ifdef STK_ALS_FIR
...
/*static int32_t stk3abx_set_irs_it_slp(struct stk3abx_data *ps_data, uint16_t *slp_time)
{
        ...
        ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, STK_ALSCTRL_REG, irs_alsctrl);
        if (ret < 0)
        {
                printk(KERN_ERR "%s: write i2c error\n", __func__);
                return ret;
...
// #ifdef STK_TUNE0
                ...
                // }
// #endif
                //stk3abx_enable_ps(ps_data, 0, 1);
                //re_enable_ps = true;
        // }

        //ret = stk3abx_set_irs_it_slp(ps_data, &irs_slp_time);
       // if(ret < 0)
         //       goto irs_err_i2c_rw;
        ...
        ret = stk3abx_set_state(ps_data, w_reg);
        if(ret < 0)
                goto irs_err_i2c_rw;
        msleep(irs_slp_time);
        ...
        ret = stk3abx_set_flag(ps_data, ret, STK_FLG_IR_RDY_MASK);
    if (ret < 0)
                goto irs_err_i2c_rw;

        ret = stk3abx_i2c_read_data(ps_data->client, STK_DATA1_IR_REG, 2, &value[0]);
        if(ret < 0)
        {
                printk(KERN_ERR "%s fail, ret=0x%x", __func__, ret);
                goto irs_err_i2c_rw;
...
#ifdef STK_CHK_REG
static int stk3abx_chk_reg_valid(struct stk3abx_data *ps_data) 
{
        unsigned char value[10];
        int err;
        /*
        ...
        for(cnt=0;cnt<9;cnt++)
        {
                value[cnt] = stk3abx_i2c_smbus_read_byte_data(ps_data->client, (cnt+1));
                if(value[cnt] < 0)
                {
        ...
        */
        err = stk3abx_i2c_read_data(ps_data->client, STK_PSCTRL_REG, 9, &value[0]);
        if(err < 0)
        {
                printk(KERN_ERR "%s: fail, ret=%d\n", __func__, err);
                return err;
        ...
        if(value[4] != ps_data->wait1_reg)
        {
                printk(KERN_ERR "%s: invalid reg 0x05=0x%2x\n", __func__, value[4]);
                return 0xFF;
        }
		
		if(value[5] != ps_data->wait2_reg)
        {
                printk(KERN_ERR "%s: invalid reg 0x05=0x%2x\n", __func__, value[5]);
                return 0xFF;
        }
		
        if(value[6] != ((ps_data->ps_thd_h & 0xFF00) >> 8))
        {
                printk(KERN_ERR "%s: invalid reg 0x06=0x%2x\n", __func__, value[6]);
                return 0xFF;
        }
        if(value[7] != (ps_data->ps_thd_h & 0x00FF))
        {
                printk(KERN_ERR "%s: invalid reg 0x07=0x%2x\n", __func__, value[7]);
                return 0xFF;
        }
        if(value[8] != ((ps_data->ps_thd_l & 0xFF00) >> 8))
        {
                printk(KERN_ERR "%s: invalid reg 0x08=0x%2x\n", __func__, value[8]);
                return 0xFF;
        }
        if(value[9] != (ps_data->ps_thd_l & 0x00FF))
        {
...
static int stk3abx_validate_n_handle(struct i2c_client *client) 
{
        ...
        err = stk3abx_chk_reg_valid(ps_data);
        if(err < 0)
        {
                printk(KERN_ERR "stk3abx_chk_reg_valid fail: %d\n", err);        
                return err;
        }
        ...
        if(err == 0xFF)
        {
                ...
                err = stk3abx_software_reset(ps_data); 
                if(err < 0)
                        return err;
                err = stk3abx_init_all_reg(ps_data);
                if(err < 0)
                        return err;
...
{
        struct stk3abx_data *ps_data =  dev_get_drvdata(dev);
    int32_t reading;

    reading = stk3abx_get_als_reading(ps_data);
...
static ssize_t stk_als_lux_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    int32_t als_reading;
        uint32_t als_lux;
    als_reading = stk3abx_get_als_reading(ps_data);    
...
static ssize_t stk_als_ir_code_show(struct device *dev, struct device_attribute *attr, char *buf)
{
        struct stk3abx_data *ps_data =  dev_get_drvdata(dev);
    int32_t reading;
    reading = stk3abx_get_ir_reading(ps_data);
    return scnprintf(buf, PAGE_SIZE, "%d\n", reading);
}
...
static ssize_t stk_ges_code_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    ret = stk3abx_get_ges_reading(ps_data, &gest0, &gest1, &gest2);
        if(ret < 0)
                return ret;
        else if(ret == 0xFFFF)
...
sz1001:~/workspace/gavin/project_A13_one/sprd_android_13(A581_01B_SC9832E_DRV)$ vi bsp/modules/kernel5.4/input/misc/stk3abx/stk3abx.c
    ...
    printk(KERN_INFO "%s: Enable GES : %d\n", __func__, (int)value);
        ...
        case 0:
                ...
                if(ps_data->ges_enabled == 1)
                        ...
                        stk3abx_enable_ges(ps_data, 0, 2);
                mutex_unlock(&ps_data->io_lock);
                break;
        case 1:
                mutex_lock(&ps_data->io_lock);
        ...
        case 2:
                mutex_lock(&ps_data->io_lock);
                stk3abx_enable_ges(ps_data, 1, 2);
                mutex_unlock(&ps_data->io_lock);
                break;
        default:
                printk(KERN_ERR "%s, invalid value %d\n", __func__, *buf);
...
static ssize_t stk_ps_enable_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    mutex_lock(&ps_data->io_lock);
    stk3abx_enable_ps(ps_data, en, 1);
    mutex_unlock(&ps_data->io_lock);
    return size;
}
...
static ssize_t stk_ps_enable_aso_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, STK_STATE_REG, w_state_reg);
    if (ret < 0)
        {
                printk(KERN_ERR "%s: write i2c error\n", __func__);
                return ret;
...
static ssize_t stk_ps_offset_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    int32_t word_data;
        ...
        ret = stk3abx_i2c_read_data(ps_data->client, STK_DATA1_OFFSET_REG, 2, &value[0]);
        if(ret < 0)
        {
                printk(KERN_ERR "%s fail, ret=0x%x", __func__, ret);
                return ret;
...
static ssize_t stk_ps_offset_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
        ...
        val[1] = offset & 0x00FF;
        ret = stk3abx_i2c_write_data(ps_data->client, STK_DATA1_OFFSET_REG, 2, val);
        if(ret < 0)
        {
                printk(KERN_ERR "%s: write i2c error\n", __func__);
                return ret;
...
static ssize_t stk_ps_code_thd_l_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int32_t ps_thd_l1_reg, ps_thd_l2_reg;
        struct stk3abx_data *ps_data =  dev_get_drvdata(dev);
    ps_thd_l1_reg = stk3abx_i2c_smbus_read_byte_data(ps_data->client,STK_THDL1_PS_REG);
    if(ps_thd_l1_reg < 0)
        {
                printk(KERN_ERR "%s fail, err=0x%x", __func__, ps_thd_l1_reg);
                return -EINVAL;
        }
    ps_thd_l2_reg = stk3abx_i2c_smbus_read_byte_data(ps_data->client,STK_THDL2_PS_REG);
    if(ps_thd_l2_reg < 0)
        {
...
static ssize_t stk_ps_code_thd_l_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    stk3abx_set_ps_thd_l(ps_data, value);
    return size;
}
...
static ssize_t stk_ps_code_thd_h_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int32_t ps_thd_h1_reg, ps_thd_h2_reg;
        struct stk3abx_data *ps_data =  dev_get_drvdata(dev);
    ps_thd_h1_reg = stk3abx_i2c_smbus_read_byte_data(ps_data->client,STK_THDH1_PS_REG);
    if(ps_thd_h1_reg < 0)
        {
                printk(KERN_ERR "%s fail, err=0x%x", __func__, ps_thd_h1_reg);
                return -EINVAL;
        }
    ps_thd_h2_reg = stk3abx_i2c_smbus_read_byte_data(ps_data->client,STK_THDH2_PS_REG);
    if(ps_thd_h2_reg < 0)
        {
...
static ssize_t stk_ps_code_thd_h_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    stk3abx_set_ps_thd_h(ps_data, value);
    return size;
}
...
static ssize_t stk_all_reg_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int32_t ps_reg[0x25];
        ...
        int len = 0;
        struct stk3abx_data *ps_data =  dev_get_drvdata(dev);
		printk(KERN_INFO "%s version:20230718\n", __func__);
        for(cnt=0;cnt<0x20;cnt++)
        {
                ps_reg[cnt] = stk3abx_i2c_smbus_read_byte_data(ps_data->client, (cnt));
                if(ps_reg[cnt] < 0)
                {
        ...
        ps_reg[cnt] = stk3abx_i2c_smbus_read_byte_data(ps_data->client, STK_PDT_ID_REG);
        if(ps_reg[cnt] < 0)
        {
                printk( KERN_ERR "%s fail, ret=%d", __func__, ps_reg[cnt]);
                return -EINVAL;
		...
		ps_reg[cnt] = stk3abx_i2c_smbus_read_byte_data(ps_data->client, 0x4E);
        if(ps_reg[cnt] < 0)
        {
                printk( KERN_ERR "%s fail, ret=%d", __func__, ps_reg[cnt]);
                return -EINVAL;
		...
		ps_reg[cnt] = stk3abx_i2c_smbus_read_byte_data(ps_data->client, 0xA1);
        if(ps_reg[cnt] < 0)
        {
                printk( KERN_ERR "%s fail, ret=%d", __func__, ps_reg[cnt]);
                return -EINVAL;
		...
		ps_reg[cnt] = stk3abx_i2c_smbus_read_byte_data(ps_data->client, 0xDB);
        if(ps_reg[cnt] < 0)
        {
                printk( KERN_ERR "%s fail, ret=%d", __func__, ps_reg[cnt]);
                return -EINVAL;
		...
		cnt++;
        ps_reg[cnt] = stk3abx_i2c_smbus_read_byte_data(ps_data->client, STK_RSRVD_REG);
        if(ps_reg[cnt] < 0)
        {
                printk( KERN_ERR "%s fail, ret=%d", __func__, ps_reg[cnt]);
                return -EINVAL;
...
static ssize_t stk_recv_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    unsigned long value = 0;
        ...
        recv_data = stk3abx_i2c_smbus_read_byte_data(ps_data->client,value);
...
static ssize_t stk_send_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
        ...
        ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, (unsigned char)addr, (unsigned char)cmd);
        if (0 != ret)
        {
                printk(KERN_ERR "%s: stk3abx_i2c_smbus_write_byte_data fail\n", __func__);
                return ret;
        }
...
static ssize_t stk_ps_cali_show(struct device *dev, struct device_attribute *attr, char *buf)
{
        ...
        ret = stk3abx_i2c_read_data(ps_data->client, 0x20, 2, &value[0]);
        if(ret < 0)
        {
                printk(KERN_ERR "%s fail, ret=0x%x", __func__, ret);
                return ret;
        ...
        ret = stk3abx_i2c_read_data(ps_data->client, 0x22, 2, &value[0]);
        if(ret < 0)
        {
                printk(KERN_ERR "%s fail, ret=0x%x", __func__, ret);
                return ret;
...
static int stk_ps_val(struct stk3abx_data *ps_data)
{
        ...
        ret = stk3abx_i2c_read_data(ps_data->client, 0x20, 4, value);
        if(ret < 0)
        {
                printk(KERN_ERR "%s fail, ret=0x%x", __func__, ret);
                return ret;
...
static int stk_ps_tune_zero_final(struct stk3abx_data *ps_data)
{
        int ret;
        ...
        ret = stk3abx_i2c_smbus_write_byte_data(ps_data->client, STK_STATE_REG, 0);
        if (ret < 0)
        {
                printk(KERN_ERR "%s: write i2c error\n", __func__);
                return ret;
...
static int32_t stk_tune_zero_get_ps_data(struct stk3abx_data *ps_data)
{
        uint32_t ps_adc;
        int ret;
        ...
        ps_adc = stk3abx_get_ps_reading(ps_data);
...
static int stk_ps_tune_zero_init(struct stk3abx_data *ps_data)
{
        int32_t ret = 0;
        uint8_t w_state_reg;
...
static int stk_ps_tune_zero_func_fae(struct stk3abx_data *ps_data)
{
        ...
        if(ret == 0)
        {
                ret = stk3abx_i2c_read_data(ps_data->client, 0x11, 2, &value[0]);
                if(ret < 0)
                {
                        printk(KERN_ERR "%s fail, ret=0x%x", __func__, ret);
                        return ret;
...
{
        struct stk3abx_data *ps_data = container_of(work, struct stk3abx_data, stk_ps_tune0_work);
...
{
        struct stk3abx_data *ps_data = container_of(timer, struct stk3abx_data, ps_tune0_timer);
...
{
        struct stk3abx_data *ps_data = container_of(timer, struct stk3abx_data, als_timer);
...
{
        struct stk3abx_data *ps_data = container_of(work, struct stk3abx_data, stk_als_work);
        ...
        flag_reg = stk3abx_get_flag(ps_data);
        if(flag_reg < 0)
                return;
...
{
        struct stk3abx_data *ps_data = container_of(timer, struct stk3abx_data, ps_timer);
...
{
        struct stk3abx_data *ps_data = container_of(work, struct stk3abx_data, stk_ps_work);
        uint32_t reading;
        int32_t near_far_state;
    ...
    //uint8_t disable_flag = 0;
        ...
        if(ps_data->ges_enabled == 2)
        {
                ...
                if(disable_flag2)
                {
                        ret = stk3abx_set_flag2(ps_data, org_flag2_reg, disable_flag2);
                        if(ret < 0)
                                goto err_i2c_rw;
                }
        }
        ...
        if(ps_data->ps_enabled)
        {
                ...
                // if(!(ps_data->psi_set))
                        // return;
#endif
                org_flag_reg = stk3abx_get_flag(ps_data);
                if(org_flag_reg < 0)
                        goto err_i2c_rw;
                ...
                // ret = stk3abx_set_flag(ps_data, org_flag_reg, disable_flag);
                // if(ret < 0)
                        // goto err_i2c_rw;
                return;
        }
...
static void stk_work_func(struct work_struct *work)
{
    ...
    uint8_t org_flag_reg;
        ...
        uint32_t nLuxIndex;
#endif
        struct stk3abx_data *ps_data = container_of(work, struct stk3abx_data, stk_work);
        int32_t near_far_state;
    ...
    if (org_flag_reg & STK_FLG_ALSINT_MASK)
    {
        ...
        reading = stk3abx_get_als_reading(ps_data);
                if(reading < 0)
                {
                        printk(KERN_ERR "%s: stk3abx_get_als_reading fail, ret=%d", __func__, reading);
                        goto err_i2c_rw;
                }
#ifndef CONFIG_STK_PS_ALS_USE_CHANGE_THRESHOLD
        nLuxIndex = stk_get_lux_interval_index(reading);
        stk3abx_set_als_thd_h(ps_data, code_threshold_table[nLuxIndex]);
        stk3abx_set_als_thd_l(ps_data, code_threshold_table[nLuxIndex-1]);
#else
...
{
        struct stk3abx_data *pData = data;
        disable_irq_nosync(irq);
        queue_work(pData->stk_wq,&pData->stk_work);
        return IRQ_HANDLED;
}
#endif  /*      #if (!defined(STK_POLL_PS) || !defined(STK_POLL_ALS))   */
static int32_t stk3abx_init_all_setting(struct i2c_client *client, struct stk3abx_platform_data *plat_data)
{
        int32_t ret;
        struct stk3abx_data *ps_data = i2c_get_clientdata(client);

        stk3abx_proc_plat_data(ps_data, plat_data);
        ret = stk3abx_software_reset(ps_data); 
        if(ret < 0)
                return ret;

        ret = stk3abx_check_pid(ps_data);
        if(ret < 0)
                return ret;
        ret = stk3abx_init_all_reg(ps_data);
        if(ret < 0)
                return ret;
...
#if (!defined(STK_POLL_PS) || !defined(STK_POLL_ALS))
static int stk3abx_setup_irq(struct i2c_client *client)
{
        int irq, err = -EIO;
        struct stk3abx_data *ps_data = i2c_get_clientdata(client);
...
static int stk3abx_suspend(struct device *dev)
{
        struct stk3abx_data *ps_data = dev_get_drvdata(dev);
        //int err;
#ifndef STK_POLL_PS
    struct i2c_client *client = to_i2c_client(dev);
...
#endif
#ifdef STK_CHK_REG
        err = stk3abx_validate_n_handle(ps_data->client);
        if(err < 0)
        {
                printk(KERN_ERR "stk3abx_validate_n_handle fail: %d\n", err); 
        }
        ...
        {
#ifdef STK_POLL_PS
               //ywq wake_lock(&ps_data->ps_nosuspend_wl);
				__pm_wakeup_event(ps_data->ps_nosuspend_wl, 3*HZ);
#else
...
static int stk3abx_resume(struct device *dev)
{
        struct stk3abx_data *ps_data = dev_get_drvdata(dev);
        //int err;
#ifndef STK_POLL_PS         
    struct i2c_client *client = to_i2c_client(dev);
        ...
        {
#ifdef STK_POLL_PS
                //ywqwake_unlock(&ps_data->ps_nosuspend_wl);
				__pm_wakeup_event(ps_data->ps_nosuspend_wl, 3*HZ);
#else
                if(device_may_wakeup(&client->dev))
                {
...
static const struct dev_pm_ops stk3abx_pm_ops = {
        SET_SYSTEM_SLEEP_PM_OPS(stk3abx_suspend, stk3abx_resume)
};

//#ifdef CONFIG_HAS_EARLYSUSPEND
#if 0
static void stk3abx_early_suspend(struct early_suspend *h)
{

}

static void stk3abx_late_resume(struct early_suspend *h)
{
...
#ifdef STK_QUALCOMM_POWER_CTRL
static int stk3abx_power_ctl(struct stk3abx_data *data, bool on)
{
        int ret = 0;
        ...
        if (!on && data->power_enabled) {
                ...
                printk(KERN_INFO "%s: disable stk3abx power", __func__);
                dev_dbg(&data->client->dev, "stk3abx_power_ctl on=%d\n",
                                on);
                ...
                data->power_enabled = on;
                printk(KERN_INFO "%s: enable stk3abx power", __func__);
                dev_dbg(&data->client->dev, "stk3abx_power_ctl on=%d\n",
                                on);
        } else {
...
static int stk3abx_power_init(struct stk3abx_data *data, bool on)
{
        int ret;
        ...
        if (!on) {
                if (regulator_count_voltages(data->vdd) > 0)
                        regulator_set_voltage(data->vdd,
                                        0, STK3ABX_VDD_MAX_UV);

                regulator_put(data->vdd);

                if (regulator_count_voltages(data->vio) > 0)
                        regulator_set_voltage(data->vio,
                                        0, STK3ABX_VIO_MAX_UV);
...
reg_vdd_set:
        if (regulator_count_voltages(data->vdd) > 0)
                regulator_set_voltage(data->vdd, 0, STK3ABX_VDD_MAX_UV);
reg_vdd_put:
        regulator_put(data->vdd);
        return ret;
}
...
static int stk3abx_device_ctl(struct stk3abx_data *ps_data, bool enable)
{
        ...
        if (enable && !ps_data->power_enabled) {
                ret = stk3abx_power_ctl(ps_data, true);
                if (ret) {
                        dev_err(dev, "Failed to enable device power\n");
                        goto err_exit;
                }
                ret = stk3abx_init_all_setting(ps_data->client, ps_data->pdata);
                if (ret < 0) {
                        stk3abx_power_ctl(ps_data, false);
        ...
        #endif
                        ret = stk3abx_power_ctl(ps_data, false);
                        if (ret) {
...
#ifdef CONFIG_OF
static int stk3abx_parse_dt(struct device *dev,
                        struct stk3abx_platform_data *pdata)
{
#if 1
        int rc;
        ...
        rc = of_property_read_u32(np, "stk,wait1-reg", &temp_val);
        if (!rc)
                pdata->wait1_reg = (u8)temp_val;
        else {
                dev_err(dev, "Unable to read wait1-reg\n");
                return rc;
        }

		rc = of_property_read_u32(np, "stk,wait2-reg", &temp_val);
        if (!rc)
                pdata->wait2_reg = (u8)temp_val;
        else {
                dev_err(dev, "Unable to read wait2-reg\n");
                return rc;
        }
...
static int stk3abx_set_wq(struct stk3abx_data *ps_data)
{
#ifdef STK_POLL_ALS
...
/*
static int stk3abx_pls_create_sysfs(struct i2c_client *client)
{
	int err;
...
static int stk3abx_set_input_devices(struct i2c_client *client, struct stk3abx_data *ps_data)
{
        int err;
		printk(KERN_INFO "%s\n", __func__);

		err = misc_register(&stk3abx_pls_device);
		if (err) {
        ...
        //stk3abx_pls_create_sysfs(client);
...
static int stk3abx_probe(struct i2c_client *client,
                        const struct i2c_device_id *id)
{
    int err = -ENODEV;
    struct stk3abx_data *ps_data;
        struct stk3abx_platform_data *plat_data;
    ...
    if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C))
    {
        ...
        ps_data = kzalloc(sizeof(struct stk3abx_data),GFP_KERNEL);
        if(!ps_data)
        {
                printk(KERN_ERR "%s: failed to allocate stk3abx_data\n", __func__);
                return -ENOMEM;
        }
...
//       wake_lock_init(&ps_data->ps_nosuspend_wl,WAKE_LOCK_SUSPEND, "stk_nosuspend_wakelock");
        ...
        if (client->dev.of_node) {
                ...
                plat_data = devm_kzalloc(&client->dev,
                        sizeof(struct stk3abx_platform_data), GFP_KERNEL);
                if (!plat_data) {
                        dev_err(&client->dev, "Failed to allocate memory\n");
                        return -ENOMEM;
                }

                err = stk3abx_parse_dt(&client->dev, plat_data);
//huanggq
                  plat_data->state_reg = stk3abx_pfdata.state_reg; 
                  plat_data->psctrl_reg = stk3abx_pfdata.psctrl_reg;
                  plat_data->alsctrl_reg = stk3abx_pfdata.alsctrl_reg; 
                  plat_data->ledctrl_reg = stk3abx_pfdata.ledctrl_reg; 
                  plat_data->wait1_reg = stk3abx_pfdata.wait1_reg;
				  plat_data->wait2_reg = stk3abx_pfdata.wait2_reg;
                  plat_data->ps_thd_h = stk3abx_pfdata.ps_thd_h; 
                  plat_data->ps_thd_l = stk3abx_pfdata.ps_thd_l;
                  plat_data->int_pin = stk3abx_pfdata.int_pin;
                  plat_data->transmittance = stk3abx_pfdata.transmittance;
//end
                dev_err(&client->dev,
                        "%s: stk3abx_parse_dt ret=%d\n", __func__, err);
                if (err)
                        return err;
        } else {
                printk(KERN_INFO "%s: probe with platform data\n", __func__);
#ifdef SPREADTRUM_PLATFORM
                plat_data = &stk3abx_pfdata;
#else
		...
		stk3abx_i2c_client = client;
		printk(KERN_INFO "%s: I2C addr = %s\n", __func__, client->addr);	
        stk3abx_set_wq(ps_data);
#ifdef QUALCOMM_PLATFORM
        ps_data->ps_thd_h = 0;
        ps_data->ps_thd_l = 0;
#endif
...
#ifdef STK_QUALCOMM_POWER_CTRL
        err = stk3abx_power_init(ps_data, true);
        if (err)
                goto err_init_all_setting;

        err = stk3abx_power_ctl(ps_data, true);
        if (err)
                goto err_power_on;
        ...
        err = stk3abx_init_all_setting(client, plat_data);
        if(err < 0)
                goto err_init_all_setting;

        err = stk3abx_set_input_devices(client, ps_data);
        if(err < 0)
                goto err_setup_input_device;

#if (!defined(STK_POLL_ALS) || !defined(STK_POLL_PS))
        err = stk3abx_setup_irq(client);
        if(err < 0)
                goto err_stk3abx_setup_irq;
#endif
...
//#ifdef CONFIG_HAS_EARLYSUSPEND
        ...
        ps_data->stk_early_suspend.suspend = stk3abx_early_suspend;
        ps_data->stk_early_suspend.resume = stk3abx_late_resume;
        register_early_suspend(&ps_data->stk_early_suspend);
#endif

#ifdef STK_QUALCOMM_POWER_CTRL
        /* enable device power only when it is enabled */
        err = stk3abx_power_ctl(ps_data, false);
        if (err)
                goto err_stk3abx_setup_irq;
#endif
...
err_setup_input_device:
        ...
        stk3abx_power_ctl(ps_data, false);
err_power_on:
        stk3abx_power_init(ps_data, false);
#endif
...
#endif
		stk3abx_i2c_client = NULL;  
err_als_input_allocate:
#ifdef STK_POLL_PS
    //ywqwake_lock_destroy(&ps_data->ps_nosuspend_wl);
#endif
    //ywqwake_lock_destroy(&ps_data->ps_wakelock);
...
static int stk3abx_remove(struct i2c_client *client)
{
        struct stk3abx_data *ps_data = i2c_get_clientdata(client);
        device_init_wakeup(&client->dev, false);
#if (!defined(STK_POLL_ALS) || !defined(STK_POLL_PS))
...
static struct i2c_driver stk_ps_driver =
{
    ...
    .probe = stk3abx_probe,
    .remove = stk3abx_remove,
    .id_table = stk_ps_id,
};


static int __init stk3abx_init(void)
{
        int ret;
...
static void __exit stk3abx_exit(void)
{
    i2c_del_driver(&stk_ps_driver);
}

module_init(stk3abx_init);
module_exit(stk3abx_exit);
MODULE_AUTHOR("Lex Hsieh <lex_hsieh@sensortek.com.tw>");
MODULE_DESCRIPTION("Sensortek stk3abx Proximity Sensor driver");
MODULE_LICENSE("GPL");
